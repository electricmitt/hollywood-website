...
from .extensions.oidc_auth import (
    OidcAuthExtensionConfig,
    get_oidc_auth_config,
)
from .extensions.riff_auth_custom_token import get_riff_auth_custom_token_config
...
class Config(BaseModel):
    ...
    IS_AGENT_BUILDER_TEMPLATE: bool = False
...
def get_oidc_auth_extension_config(cfg: Config) -> StackAuthExtensionConfig | None:
    extension = get_extension(cfg, ExtensionType.oidc_auth)
    if not extension or not extension.config:
        return None
    return OidcAuthExtensionConfig(**extension.config)
...
def parse_auth_configs(cfg: Config) -> list[AuthConfig]:
    ...
    # Add oidc auth config if extension is enabled
    if oidc_auth_cfg := get_oidc_auth_extension_config(cfg):
        auth_configs.append(get_oidc_auth_config(oidc_auth_cfg))
    ...
    # Add riff-auth-custom-token config
    auth_configs.append(
        get_riff_auth_custom_token_config(
