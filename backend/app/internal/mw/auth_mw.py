...
def authorize_jwt_token(
    ...
    # TODO: Reorganize auth_configs as a dict[issuer, list[auth_cfg]] everywhere
    configs_matched: list[AuthConfig] = [
        auth_cfg for auth_cfg in auth_configs if auth_cfg.issuer == token_iss
    ]
    if not configs_matched:
        if audit_log:
            audit_log(f"No matching auth config for token issuer {token_iss}")
        return None

    for auth_cfg in configs_matched:
        # Verify audience unless both config and token has it set to None
        verify_aud = bool(token_aud or auth_cfg.audience)

        # Get the audience from config that the token aud is matching
        expected_audience = determine_expected_audience(token_aud, auth_cfg, url)

        # If we want to verify audience, but got no expected audience, fail
        ...
        # Skipping audience verification MUST be accompanied by required attributes
        if not verify_aud and len(auth_cfg.required_attributes) == 0:
            if audit_log:
                audit_log(
                    f"Auth misconfiguration, missing aud or required attributes for {auth_cfg.issuer}"
                )
            continue

        # If we deliberately skip audience verification, we need to disable it.
        # (This is for workos which doesn't set aud)
        opts = options.copy() if options else {}
        opts["verify_aud"] = verify_aud

        payload = validate_token(token, expected_audience, auth_cfg, opts, audit_log)
        if payload is None:
            if audit_log:
                audit_log("Failed to validate token")
            continue

        # Verify additional custom attributes
        required_attributes_ok = True
        for k, v in auth_cfg.required_attributes:
            if k not in payload:
                if audit_log:
                    audit_log(
                        f"Missing required attribute {k} in token payload {payload}"
                    )
                required_attributes_ok = False
                break
            if payload[k] != v:
                if audit_log:
                    audit_log(
                        f"Required attribute {k} mismatch in token payload {payload}"
                    )
                required_attributes_ok = False
                break
        if not required_attributes_ok:
            continue
